...
def full_training(feat_cfg, base_path, drive_dir):
    ...
    for d in depths:
        ...
        for network in networks:
            ...
            train_list, val_list, test_list = split_spatial_files(all_file_paths, df_master, d, horizons, lookbacks, feat_cfg)
...
#                 df = load_csv(csv_path, df_master, depth)
...
# %%
def get_osiris_data(osiris_dir: str, feat_cfg: dict) -> List[pd.DataFrame]:
    ...
    for _, row in sites.iterrows():
        ...
        for sonde_file in sonde_files:
            ...
            # Feature engineering
            df_sonde = engineer_features(df_sonde, feat_cfg)
            all_dfs.append(df_sonde)
    print(all_dfs[0].columns)
    return all_dfs

# %%
def run_osiris_evaluation(model, scaler_x, scaler_y, lst_local, model_dir, lookback, horizon, depth, nb_windows=None, model_name="unknown", network_str="", feat_cfg=None):
    """
    Évalue les données osiris et calcule les métriques.
    Trace les deux prédictions sur le même graphique.
    ...
    for i, (df_L) in enumerate(lst_local):
        ...
        if col_name in df_L.columns:
            df_L[col_name] = df_L[col_name]/100
            df_L.rename(columns={col_name: 'soil_moisture'}, inplace=True)
        df_L = update_soil_name(df_L, depth)
        
        segments = cut_timeseries(df_L, min_length=lookback + horizon + 1, feat_cfg=feat_cfg)
        file_y_true, file_y_pred, file_dates = [], [], []

        for seg in segments:
            X_seg, y_seg, d_seg = make_supervised([seg], scaler_x, scaler_y, lookback, horizon, feat_cfg, model_name)
        ...
        if SAVE_RESULTS_CSV:
            ...
            for metricname, val_L, prec in [
                ('rmse_osiris', rmse_h, 6),
                ('ubrmse_osiris', ubrmse_h, 6),
                ('smape_osiris', smape_h, 2),
                ('r2_osiris', r2_h, 4),
                ('kge_osiris', kge_h, 4),
                ('corr_osiris', corr_h, 4)
            ]:
                save_to_results_csv({'type': 'OSIRIS' + (f"_{probe_name}" if probe_name else ""), 
...
def osiris_fine_tuning(feat_cfg, base_path, drive_dir, all_dfs):
    ...
    site_ids = []
    for site in all_dfs:
        site_ids.append(site['site_id'].iloc[0])
    #unique id
    ...
    for d in depths:
        ...
        # Osiris
        # 1. Split spatial des fichiers 
        # format de site_id : "site_0001" à "site_0016" donc si <= 8 alors train, sinon val/test
        train_list_osiris, val_list_osiris, test_list_osiris = [], [], []
        #  Mélange aléatoire des sites pour éviter un biais
        ...
        for df in all_dfs:
            ...
            if df['site_id'].iloc[0] in site_ids[:n_train]:
                train_list_osiris.append(df)
            elif df['site_id'].iloc[0] in site_ids[n_train:n_train + n_val]:
                val_list_osiris.append(df)
            else:
                test_list_osiris.append(df)

        print(f"Spatial split: train={len(train_list_osiris)} files, val={len(val_list_osiris)} files, test={len(test_list_osiris)} files")
                ...
                for horizon in horizons:
                    ...
                    for NB in nb_windows:
                        ...
                        for m in model_name:
                            ...
                            ### Fine-tuning sur Osiris
                            is_dl = m in ["convlstm", "lstm", "transformer", "gru", "tcn"]
                            output_dir_ft = output_dir + "_fine_tuned"
                            if model_ft is not None:
                                fit_scalers(train_list_osiris, scaler_x_ft, scaler_y_ft, feat_cfg)
                                X_ft, y_ft, _ = make_supervised(train_list_osiris, scaler_x_ft, scaler_y_ft, LB, horizon, feat_cfg, m)
                                X_val_ft, y_val_ft, _ = make_supervised(val_list_osiris, scaler_x_ft, scaler_y_ft, LB, horizon, feat_cfg, m)
                                ...
                                if is_dl:
                                    ...
                                    model_ft.fit(X_ft, y_ft, validation_data=(X_val_ft, y_val_ft),
                                                epochs=50, batch_size=BATCH_SIZE, callbacks=cbs, verbose=1)
                                else:
